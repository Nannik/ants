...
export class AnimatedCharacter extends gfx.Node3
{
    ...
    private addBoneRecursively(boneName: string, parentNode: gfx.Node3, boneSpaceToParentBoneSpace: gfx.Matrix4): void
    {
		...
		const direction = this.boneData.boneDirection(boneName);
		const length = this.boneData.boneLength(boneName);
		const matrix = gfx.Matrix4.makeTranslation(gfx.Vector3.multiplyScalar(direction, length));

        for (let i = 0; i < this.boneData.numChildren(boneName); i++) {
            const childBoneName = this.boneData.childBoneName(boneName, i);

            this.addBoneRecursively(childBoneName, bone, matrix)
        }
    }
    ...
    private computePose(currentFrame: number, overlayFrame: number): Pose
    {
        ...
        if (this.overlayQueue.length > 0) {
            ...
            if (!this.useAbsolutePosition) {
                const relativeOverlayPosition = gfx.Vector3.clone(this.overlayQueue[0].frames[overlayFrame].rootPosition);
                relativeOverlayPosition.subtract(this.overlayPose.rootPosition);

                const relativePosition = gfx.Vector3.clone(this.clip!.frames[currentFrame].rootPosition);
